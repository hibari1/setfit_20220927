...
class SetFitTrainer:
    def __init__(
        ...
        batch_size: int = 16,
        classifer_path: str = "",
        info: str = "added: eval (f1, cr, cm), classifer (save, load, predict)",
    ):
    ...
    def evaluate(self):
        ...
        if self.metric == "f1":
            score = metric_fn.compute(predictions=y_pred, references=y_test, average="weighted")
        else:
            score = metric_fn.compute(predictions=y_pred, references=y_test)
    ...
    def save_classifier(self):
        self.model._save_pretrained(self.classifer_path)
