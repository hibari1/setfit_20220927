...
style:
	python -m black --line-length 119 --exclude="scripts/adapet|scripts/tfew" --target-version py38 $(check_dirs)
	python -m isort --skip scripts/adapet --skip scripts/tfew $(check_dirs)

quality:
	python -m black --check --line-length 119 --exclude="scripts/adapet|scripts/tfew" --target-version py38 $(check_dirs)
